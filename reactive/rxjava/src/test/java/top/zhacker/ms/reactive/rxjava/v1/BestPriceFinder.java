...
import static top.zhacker.ms.reactive.rxjava.v1.ExchangeService.Money;
...
//                                                   new Shop("ShopEasy"));
    ...
    {
        IntStream.range(1,100).forEach((i)-> shops.add(new Shop("shop"+i)));
    }
    ...
    public Observable<String> findPricesObservable(String product){
        return Observable.from(shops)
                .map(shop -> shop.getName() + " price is " + shop.getPrice(product));
    }
